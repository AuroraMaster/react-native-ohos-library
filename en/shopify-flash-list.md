...
| extraData                              | A marker property for telling the list to re-render                                                                                                                                                                        | any                 | No       | All      | Yes                |
...
| viewabilityConfig                      | A default configuration for determining whether items are viewable. Supports fields such as `minimumViewTime`, `viewAreaCoveragePercentThreshold`, `itemVisiblePercentThreshold`, and `waitForInteraction`. Dynamically changing this configuration at runtime is not supported.                                                                                                                                                                       | object              | No       | All      | Yes                |
| renderItem<sup>changed in 2.1.1</sup>                             | Takes an item from data and renders it into the list. The callback parameter `info` contains `item`, `index`, `target`, and `extraData` fields. `target` is a render target enum with possible values: `"Cell"` (list item), `"StickyHeader"` (sticky header), and `"Measurement"` (size measurement, deprecated in v2).                                                                                                                                                                                                          | function            | Yes      | All      | Yes                |
| data                                   | For simplicity, data is a plain array of items of a given type.                                                                                                                                                                                                | ItemT[]             | Yes      | All      | Yes                |
| CellRendererComponent                  | Each cell is rendered using this element. Can be a React Component Class, or a render function                                                                                                                                                                 | JSX Element         | No       | All      | Yes                |
| ListFooterComponent                    | Rendered at the bottom of all the items. Can be a React Component Class or a React Element.                                                                                                                                                                                                                        | JSX Element         | No       | All      | Yes                |
| ListHeaderComponent                    | Rendered at the top of all the items. Can be a React Component Class or a React Element.                                                                                                                                                                                                                           | JSX Element         | No       | All      | Yes                |
| refreshControl                         | A custom refresh control element.                                                                                                                                                                                                                              | JSX Element         | No       | All      | Yes                |
| renderScrollComponent                  | Rendered as the main scrollview.                                                                                                                                                                                                                               | JSX Element         | No       | All      | Yes                |
...
| onViewableItemsChanged                 | Called when the viewability of rows changes, as defined by the viewabilityConfig prop. The callback parameters include `viewableItems` and `changed` arrays, with elements being `ViewToken` objects (containing `item`, `key`, `index`, `isViewable`, and `timestamp` fields).                                                                                                                                                                         | callback            | No       | All      | Yes                |
| getItemType                            | Allows developers to specify item types to optimize recycling. Accepts `item`, `index`, and `extraData` parameters and returns a type identifier. Items of the same type are recycled with each other, with the default type being 0.                                                                                                                                                                                                                       | function            | No       | All      | Yes                |
| overrideItemLayout<sup>changed in 2.1.1</sup>                     | Used to change item column span. Accepts `layout`, `item`, `index`, `maxColumns`, and `extraData` parameters. In v1, supports both `layout.span` and `layout.size`; in v2, only `layout.span` is supported.                                                                                                                                                                   | function            | No       | All      | Yes                |
| ItemSeparatorComponent                 | Rendered in between each item, but not at the top or bottom. Provides `leadingItem` and `trailingItem` props by default.                                                                                                                                                                                                   | JSX Element         | No       | All      | Yes                |
| ListEmptyComponent                     | Rendered when the list is empty. Can be a React Component Class or a React Element.                                                                                                                                                                                                                                | JSX Element         | No       | All      |Yes               |
| ListFooterComponentStyle               | Styling for internal View for ListFooterComponent.                                                                                                                                                                                                             | StyleProp | No       | All      | Yes                |
...
| onLoad                                 | This event is raised once the list has drawn items on the screen. The callback parameter `elapsedTimeInMs` is the elapsed time in milliseconds.                                                                                                                                                                                              | callback            | No       | All      | Yes               |
...
| viewabilityConfigCallbackPairs         | List of ViewabilityConfig/onViewableItemsChanged pairs. Each pair contains `viewabilityConfig` and `onViewableItemsChanged` properties. A specific onViewableItemsChanged will be called when its corresponding ViewabilityConfig's conditions are met.                                                                                        | object              | No       | All      | Yes                 |
...
| props<sup>2.1.1</sup> | Get access to current props. | RecyclerViewProps\<T\> | No | All | Yes |
...
| useRecyclingState<sup>2.1.1</sup>| A state management Hook with automatic reset functionality. Similar to useState, but automatically resets state when specified dependencies `deps` change, and triggers an optional `onReset` callback. | Hook | No | All | Yes |
| useMappingHelper<sup>2.1.1</sup>| Returns an object containing a `getMappingKey` function, used to generate optimal keys when creating component lists with `.map` operations. Returns `index` inside FlashList to optimize recycling performance, and returns `itemKey` outside to maintain stable keys. | Hook | No | All | Yes |
...
| recordInteraction               | Tells the list an interaction has occurred, which should trigger viewability calculations, e.g. if waitForInteraction is true and the user has not scrolled. | function | No       | All      | Yes              |
| scrollToEnd                     | Scrolls to the end of the content. Supports the `animated` parameter to control whether to use animation.                                                                                                                            | function | No       | All      | Yes               |
| scrollToIndex<sup>changed in 2.1.1</sup>                  | Scroll to a given index. Supports `index`, `animated`, `viewPosition` (0=top, 0.5=center, 1=bottom), and `viewOffset` parameters. v1 and v2 versions handle the `viewOffset` parameter differently, specifically in opposite directions.                                                                                                                                     | function | No       | All      | Yes               |
| scrollToItem<sup>changed in 2.1.1</sup>                      | Scroll to a given item. Supports `item`, `animated`, `viewPosition`, and `viewOffset` parameters. v1 and v2 versions handle the `viewOffset` parameter differently, specifically in opposite directions.                                                                                                                                      | function | No       | All      | Yes              |
| scrollToOffset                  | Scroll to a specific content pixel offset in the list. Supports `offset`, `animated`, and `skipFirstItemOffset` parameters.                                                                                                        | function | No       | All      | Yes              |
| recomputeViewableItems<sup>1.8.3+</sup> | Retriggers viewability calculations. | function | No | All | Yes |
